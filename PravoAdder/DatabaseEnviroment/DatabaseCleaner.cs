...
namespace PravoAdder.DatabaseEnviroment
{
	public class DatabaseCleaner
	{
		...
		private async Task<bool> TrySendWithoutResponse(IDictionary<string, string> parameters, string uri, HttpMethod httpMethod)
		{
		...
		protected async Task<EnviromentMessage> DeleteProjectGroup(string projectGroupId)
		{
			...
			var result = await TrySendWithoutResponse(parameters, "Projects/DeleteProjectGroup/{Id}", HttpMethod.Delete);
			return result
