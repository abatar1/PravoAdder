...
{
	public class ExcelWithReferenceReader : TableReader
	{
		private class TableInfo : ExcelWithReferenceReader, IDisposable
		{
			public int TotalRows { get; }
			public int TotalColumns { get; }
			public Row Header { get; }
			public IDictionary<FieldInfo, int> HeaderRev { get; }
			public List<int> ColorHeader { get; }

			private ExcelWorksheet Worksheet { get; }
			private readonly ExcelPackage _excelPackage;

			public TableInfo(TableSettings settings, string filename)
			{
		...
		public override Table Read(TableSettings settings)
		{
			...
			for (var rowNum = settings.DataRowPosition + settings.StartRow - 1; rowNum <= mainTableInfo.TotalRows; rowNum++)
			{
